...
private: // функции в привате
	...
	void Show() {
		...
		cout << "Profit: " << calculateNetProfit(revenue, expenses) << endl; // костыль который никто не заметит (костыль, который заметили) :3
	...
	void EditEconomicalDate(vector<EconomicalDate>& EcoDate) {
		...
		if ((cases>= 0) && (cases < EcoDate.size())) {
			...
			if (newName != "0" ) {
				name = newName;
				EcoDate[cases].setCompanyName(newName);
			}
			else name = getCompanyName();

			cout <<"\n Enter new revenue or 0 if you do not want to change it"<<endl;
			cin>>newRev;
			if (newRev != 0 ) {
				rev = newRev;
				EcoDate[cases].setRevenue(newRev);
			}
			else rev = getRevenue();
			...
			if (newExp != 0 ) {
				exp = newExp;
				EcoDate[cases].setExpenses(newExp);
			}
			else exp = getExpenses();

			cout << "\n Enter new assets or 0 if you do not want to change it" << endl;
			cin >> newAss;
			if (newAss != 0 ) {
				ass = newAss;
				EcoDate[cases].setAssets(newAss);
			}
			else ass = getAssets();
			...
			if (newLiab != 0 ) {
				liab = newLiab;
				EcoDate[cases].setLiabilities(newLiab);
			}
			else liab = getLiabilities();
		}    
		else {
        		cout << "\n No data for editing was found!" << endl;
		}
		ResetDate(name, rev, exp, calculateNetProfit(rev,exp) , ass, liab);
	}
